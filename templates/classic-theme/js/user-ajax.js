...
$(function () {
    ...
    $('.category-dropdown').on('click', '#category-change a', function (ev) {
        if ("#" === $(this).attr('href')) {
            superNova = parseInt(superNova) + 1;
            ev.preventDefault();
    ...
    $(document).on('click','.all_cate_in_new_mobile',function(){
        bd_n2 = parseInt(bd_n2) + 1;
        if(bd_n2 > 1){
            $('#submit').trigger('click');
        } 
    })
    ...
    //    $('#submit').trigger('click');
    // }); Removed For Auto Refresh

    if(window.innerWidth < 770){
        $("#stack-menu").stackMenu();
        $(".category-dropdown a").removeAttr('data-toggle');
        $(".category-dropdown a").addClass('starc-menu');
        $("#stack-menu").hide();
    }
    $(document).on('click','.starc-menu',function(){
        bd_n = parseFloat(bd_n) + 1;
        if(bd_n > 1){
            $("#stack-menu").toggle();
        }
        
    });
    $(document).on('click', '#stack-menu ul li a', function (ev) {
        if ("#" === $(this).attr('href')) {
            superNova2 = parseInt(superNova2) + 1;
            ev.preventDefault();
            var parent = $('#category-change').parent();
            parent.find('.change-text').html($(this).html());
            var id = $(this).data('ajax-id');
            var type = $(this).data('cat-type');
            ...
            if($(this).parent().find('ul').length){
                ...
                if(superNova2 > 1){
                    if(!$(this).hasClass('stack-menu__link--back')){
                        $('#submit').trigger('click');
                    }
                }

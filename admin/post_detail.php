...
if(isset($_GET['resubmit'])) {
    ...
    $query2 = "SELECT * FROM ".$config['db']['pre']."product WHERE id='".$_GET['id']."' LIMIT 1";
    
    $result2 = mysqli_query($mysqli, $query2);
    
    
    if (mysqli_num_rows($result2) > 0) {
         $infoold = mysqli_fetch_assoc($result2);
         
         $olditem_title = $infoold['product_name'];
         $olditem_id    = $infoold['id'];
         $olditem_view = $infoold['view'];
         $olditem_price = $infoold['price'];
         $olditem_description = $infoold['description'];
    }
...
if (mysqli_num_rows($result) > 0) {
    ...
    $item_ad_network  = $info['ad_network'];
    $item_ad_payment = $info['ad_payment'];
    $item_refno = $info['refno'];
    $item_added_date = $info['added_date'];
    $item_added_time = $info['added_time'];  
    ...
    $item_hide_phone2 = $info['hide_phone2'];

    if($item_phone2 != "" && $item_hide_phone2 == '0'){
        $item_hide_phone2 = "No";
    }else{
        $item_hide_phone2 = "Yes";
    }
                                ...
                                <div class="tab-content">
                                    <div id="4" class="tab-pane active fade in">
                                        <div>
                                            ...
                                            <?php
                                            $item_customx = array();
                                            $item_custom_textareax = array();
                                            $item_checkboxx = array();
                                            $queryx = "SELECT * FROM `".$config['db']['pre']."old_custom_data` where product_id = '".$item_id."'";
                                            $query_resultx = mysqli_query($mysqli, $queryx);
                                            
                                            while($customdatax = mysqli_fetch_array($query_resultx)){
                                                $field_idx = $customdatax['field_id'];
                                                $field_typex = $customdatax['field_type'];
                                                $field_datax = $customdatax['field_data'];
                                                $custom_fields_titlex = get_customField_title_by_id($config,$field_idx);
                                                
                                                if($field_typex == 'checkboxes'){
                                                  $checkbox_value = explode(',', $field_datax);
                                                  $checkbox_value2 = array();
                                                    foreach ($checkbox_value as $val){
                                                            $val = get_customOption_by_id(trim($val));
                                                            $checkbox_value2[] = '<div class="col-md-4 col-sm-4"><div style="line-height: 30px;"><i class="fa fa-check"></i> '.$val.'</div></div>';
                                                    ...
                                                    if($custom_fields_title != ""){
                                                      $item_checkboxx[$field_idx]['title'] = $custom_fields_titlex;
                                                      $item_checkboxx[$field_idx]['value'] = implode('  ', $checkbox_value2);
                                                    }
                                            
                                                    }elseif($field_typex == 'textarea') {
                                                        $item_custom_textareax[$field_idx]['title'] = $custom_fields_titlex;
                                                        $item_custom_textareax[$field_idx]['value'] = stripslashes($field_datax);
                                                    }else{
                                                        if($field_typex == 'radio-buttons' or  $field_typex == 'drop-down') {
                                                            $custom_fields_data = get_customOption_by_id($field_datax);
                                                        }else{
                                                            $custom_fields_data = stripslashes($field_datax);
                                                        }
                                                    $item_customx[$field_idx]['title'] = $custom_fields_titlex;
                                                    $item_customx[$field_idx]['value'] = $custom_fields_data;
                                                    }
                                            }
                                            
                                            ?>
                                            <?php if(count($item_customx) > 0):?>
                                            <div class="quick-info">
                                                <div class="detail-title">
                                                    <h2 class="title-left">Old Additional Details</h2>
                                                </div>
                                                <ul class="clearfix">
                                                    <?php
                                                    foreach($item_customx as $value)
                                                    {
                                                        echo '<li>
                                                                    <div class="inner clearfix">
                                                                        <span class="label">'.$value['title'].'</span>
                                                  ...
                                                  <table class="table table-bordered">
                                                    ...
                                                    <tbody>
                                                      <tr class="success">
                                                        <th>Old</th>
                                                        <th><?=$olditem_title;?></th>
                                                        <td>Rs. <?=$olditem_price?></td></td>
                                                        <td><?=$olditem_description;?></td>
                                                      </tr>
                                                      <tr>
                                                        <th>New</th>
                                                        <th><?=$item_title;?></th>
                                           ...
                                           <!--End test-->
                                           <?php if($item_ad_payment != ""): ?>
                                            <table class="table"> <caption>Premium Ad Details</caption>
                                                <thead> 
                                                    <tr>
                                                      <th>Ad Network</th>
                                                 ...
                                                 <tbody> 
                                                 <tr>
                                                      <th><?=$item_ad_network;?></th>
                                                      <td><?=$item_ad_payment;?></td> 
                                                      <td><?=$item_refno;?></td> 
                                                      <td><?=$item_added_date;?></td>
                                                      <td><?=$item_added_time;?></td>
                                                  </tr>
                                                </tbody>
                                             </table>
                                            <?php endif; ?>
                            ...
                            <div class="item-box meta-attributes">
                                <div class="pad-20">
                                    <table class="meta-attributes__table align-left" cellspacing="0" cellpadding="10" border="0">
                                        ...
                                        <tr>
                                            <td class="meta-attributes__attr-name">Phone 1</td>
                                            <td class="meta-attributes__attr-detail"><?php echo $item_phone ?></td>
                                        </tr>
                                        <tr>
                                            <td class="meta-attributes__attr-name">Hide Phone 1</td>
                                            <td class="meta-attributes__attr-detail"><?php echo $item_hide_phone; ?></td>
                                        </tr>
                                        <?php if($item_phone2 != ""): ?>
                                        <tr>
                                            <td class="meta-attributes__attr-name">Phone 2</td>
                                            <td class="meta-attributes__attr-detail"><?php echo $item_phone2; ?></td>
                                        </tr>
                                        <tr>
                                            <td class="meta-attributes__attr-name">Hide Phone 2</td>
                                            <td class="meta-attributes__attr-detail"><?php echo $item_hide_phone2; ?></td>
                                        </tr>
                                        <?php endif; ?>
                                        <tr>

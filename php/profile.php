...
if(isset($_GET['username'])){
    ...
    if(is_array($get_userdata)){
        ...
        if($config['contact_validation'] == '1'){
            $user_email = (checkloggedin($config)) ? $get_userdata['email'] : "Login to see";
            $user_phone = (checkloggedin($config)) ? $get_userdata['phone'] : "Login to see";
        }else{
        ...
        $page->SetParameter ('COVER', $user_cover);
        ...
        if(check_user_upgrades($mysqli,$user_id))
        {
            $sub_info = get_user_membership_detail($mysqli,$user_id);

...
if (mysqli_num_rows($result) > 0) {
    ...
    $item_created_at = convertDateTime(strtotime($info['created_at']));
...
if (mysqli_num_rows($result1) > 0) {
    ...
    while($info1 = mysqli_fetch_assoc($result1)) {
        ...
        $item[$info1['id']]['cityname'] = get_cityName_by_id($config,$info1['city']);
        //$item[$info1['id']]['state'] = get_stateName_by_id($config,$info1['state']);
        $item[$info1['id']]['country'] = get_countryName_by_id($config,$info1['country']);
...
if(check_user_upgrades($mysqli,$item_author_id))
{
    $sub_info = get_user_membership_detail($mysqli,$item_author_id);
...
$page->SetParameter ('ITEM_PHONE2', $item_phone2);
$page->SetParameter ('ITEM_HIDE_PHONE2', $item_hide_phone2);

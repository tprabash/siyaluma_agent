...
if(isset($_POST['action'])){
    ...
    if ($_POST['action'] == "getCityidByCityName") {getCityidByCityName($con,$config);}
...
function email_verify(){
    ...
    if(checkloggedin($config))
    {
        ...
        $respond = $lang['SENT'];
        echo '<a class="uiButton uiButtonLarge resend btn btn-primary" style="box-sizing:content-box;"><span class="uiButtonText">'.$respond.'</span></a>';
        die();
...
function getCityidByCityName($con,$config)
{
    ...
    $query = "SELECT code FROM `".$config['db']['pre']."subadmin1` WHERE `name` = '$state' or `asciiname` = '$state'";
    $result = mysqli_query($con,$query);
    $total = mysqli_num_rows($result);
    if ($result = $con->query($query)) {
        ...
        $query = "SELECT id  FROM `".$config['db']['pre']."cities` WHERE  (name = '$city_name' or `asciiname` = '$city_name') AND subadmin1_code = '$state_code' AND country_code = '$country_code' LIMIT 1";
        $result = mysqli_query($con,$query);
        $total = mysqli_num_rows($result);
        if ($result = $con->query($query)) {
            $row = mysqli_fetch_assoc($result);
...
function ModelGetStateByCountryID($con,$config)
{
    ...
    if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if($count == 1)
            {
                $list .=  '<li class="selected"><a class="selectme" data-id="'.$country_id.'" data-name="'.$lang['ALL'].' '.$countryName.'" data-type="country"><strong>'.$lang['ALL'].' '.$countryName.'</strong></a></li>';
            }
    ...
    if($result){
        ...
        if ($total > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                if($count == 1)
                {
                    $list .=  '<li class="selected"><a id="changeState"><strong><i class="fa fa-arrow-left"></i>'. " Change District".'</strong></a></li>';
        ...
        echo '<ul class="column col-md-12 col-sm-12 cities">
            <li class="selected"><a id="changeState"><strong><i class="fa fa-arrow-left"> </i>'."Change District".'</strong></a></li>
...
INNER JOIN `".$config['db']['pre']."subadmin1` AS s ON s.code = c.subadmin1_code
 ...
 LIMIT 20";
    ...
    if ($total > 0) {
        ...
        echo '<ul class="searchResgeo"><li><span class="noresult">'.$lang['NO-RESULT-FOUND'].'</span></li>';
    }
}
...
function hideItem($con,$config){
    $id = $_POST['id'];
    if (trim($id) != '') {
        $query = "SELECT hide FROM ".$config['db']['pre']."product WHERE id='" . $id . "' LIMIT 1";
        $query_result = mysqli_query($con, $query);
        $info = mysqli_fetch_assoc($query_result);
        $status = $info['hide'];
        if($status == "0"){
            $timenow = date('Y-m-d H:i:s');
            $deleted_timestamp = strtotime($timenow);
            $con->query("UPDATE `".$config['db']['pre']."product` set hide='1', updated_at = '".$deleted_timestamp. "'    WHERE `id` = '".$id."' and `user_id` = '".$_SESSION['user']['id']."' ");
            //$con->query("UPDATE `".$config['db']['pre']."product` set hide='1', status='pending' WHERE `id` = '".$id."' and `user_id` = '".$_SESSION['user']['id']."' ");
            echo 1;
        }else{
            $con->query("UPDATE `".$config['db']['pre']."product` set hide='0' WHERE `id` = '".$id."' and `user_id` = '".$_SESSION['user']['id']."' ");
            //$con->query("UPDATE `".$config['db']['pre']."product` set hide='0' WHERE `id` = '".$id."' and `user_id` = '".$_SESSION['user']['id']."' ");
            echo 2;
        }
        die();
...
{
    $appr = 'active';
    $country = check_user_country($config);
    ...
    if ($city != '') {
        ...
        if ($serachStr != '') {
            $where .= "AND p.product_name LIKE '%".validate_input($serachStr)."%'";
        }

        if ($searchBox != '') {
            $where .= " AND p.category = '".validate_input($searchBox)."'";
        }

        if ($catid != '') {
            $where .= " AND p.sub_category = '".validate_input($catid)."'";
        }

        if ($country != '') {
            $where .= "AND p.country = '".validate_input($country)."'";
        }
...
{
    global $config,$con,$lang,$link,$cats;
...
LEFT JOIN `".$config['db']['pre']."usergroups` as g ON g.group_id = u.group_id
 WHERE p.product_name like '%$qString%' and p.status = 'active'
 ORDER BY
  CASE
...
//echo "<pre>".$sql."</pre>";
        ...
        if ((!empty($TAGOutput) && count($TAGOutput) > 0) || (!empty($CATOutput) && count($CATOutput) > 0) || (!empty($TagCatOutput) && count($TagCatOutput) > 0) || (!empty($TitleOutput) && count($TitleOutput) > 0)) {
            ...
            $moreResult = array();
            $mResults   = '<strong>' . $lang['MORE-RESULTS-FOR'] . '</strong>';
            $mResults .= $qString;

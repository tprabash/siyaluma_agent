...
function check_user_country($config)
{
    if(isset($_SESSION['user']['country']))
    {
        ...
        //if(isset($details->country) && $config['country_type'] == 'multi'){
            ...
            $country_code = "LKR";
            if(check_country_activated($config,$country_code)){
                $_SESSION['user']['country'] = $country_code;
                /*$latlong = $details->loc;
...
function get_user_group($con)
{
    ...
    {
        $user_info = mysqli_fetch_row(mysqli_query($con,"SELECT group_id FROM ".$config['db']['pre']."user WHERE id='".validate_input($_SESSION['user']['id'])."' LIMIT 1"));
...
function isPhoneExist($config, $user_id){
     $query = "SELECT phone FROM ".$config['db']['pre']."user WHERE id='" . $user_id . "'";
     $query_result = mysqli_query(db_connect($config), $query);
     if (mysqli_num_rows($query_result) > 0){
        $info = mysqli_fetch_assoc($query_result);
        $phone = $info['phone'];
